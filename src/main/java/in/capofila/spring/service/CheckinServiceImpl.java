...
public class CheckinServiceImpl implements CheckinService {
	...
	Scheduler sched = null;
	{
		try {
			...
			if (sched.isShutdown() || sched.isInStandbyMode()) {
				sched.start();
				logger.debug("starting Schedular now");
			}
		...
		try {
			for (String groupName : sched.getJobGroupNames()) {
				for (JobKey jobKey : sched.getJobKeys(GroupMatcher.jobGroupEquals(groupName))) {
					...
					String jobGroup = jobKey.getGroup(); // get job's trigger
					...
					for (Trigger trigger : triggers) {
						...
						/*
						 * CheckinDetails cd = new CheckinDetails(); cd.setJobName(jobName);
						 * cd.setJobGroup(jobGroup); cd.setTriggerName(trigger.getKey().getName());
						 ...
						 * cd.setLastName(checkinDetails.getLastName()); schdChekcins.add(cd);
						 */
	...
	public boolean createJob(CheckinDetails checkinDetails) {
		...
		try {
			...
			Trigger trigger = TriggerBuilder
					.newTrigger().withIdentity(triggerName, groupName).withSchedule(CronScheduleBuilder
							.cronSchedule(/*
											 * new CronExpression(checkinDetails.getSs() + " " + checkinDetails.getMm()
											 * + " " + SchedulerUtils.to24hr(checkinDetails) + " ? * * ")
											 */ /* new CronExpression("00 47 21 3 APR ? 2019") */
									new CronExpression(checkinDetails.getSs() + " " + checkinDetails.getMm() + " "
											+ " " + SchedulerUtils.to24hr(checkinDetails)+" "+checkinDetails.getDateOfMonth()+" "+SchedulerUtils.toMonth(checkinDetails)+" ? "+checkinDetails.getYyyy()))
			...
			EmailSender.sendEmail(checkinDetails.getEmail(), checkinDetails.getJobStatus(),
					SchedulerUtils.emailFormatter(checkinDetails));
		} catch (Exception e) {
			status = false;
			logger.error(e.getMessage());
		} finally {
			logger.debug("Finally job created");
		}
		return status;
	}
	...
	public static void main(String[] args) {
		...
		checkinDetails.setDateOfMonth("03");
		checkinDetails.setMonth("03");
		checkinDetails.setYyyy("2019");
		checkinDetails.setHh("08");
		checkinDetails.setMm("30");
